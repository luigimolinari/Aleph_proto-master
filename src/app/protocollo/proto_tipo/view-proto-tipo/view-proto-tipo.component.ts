...
@Component({
  selector: 'app-view-proto-tipo',
  templateUrl: './view-proto-tipo.component.html',
  styleUrls: ['./view-proto-tipo.component.css']
})
export class ViewProtoTipoComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllProtoTipo().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  cancella(id, descrizione) {
    if (confirm("Stai per disattivare " + descrizione)) {
      this.apiService.DeleteProtoTipo(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
  ...
  editMethod(e) {
    ...
    this.router.navigate(['/editprototipo'], navigationExtras1);
  }
  }
