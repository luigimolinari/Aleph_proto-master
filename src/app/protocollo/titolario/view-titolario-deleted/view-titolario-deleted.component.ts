...
@Component({
  selector: 'app-view-titolario-deleted',
  templateUrl: './view-titolario-deleted.component.html',
  styleUrls: ['./view-titolario-deleted.component.css']
})
export class ViewTitolarioDeletedComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllTitolarioDeleted().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  restore(id, descrizione) {
    if (confirm("Stai per riattivare " + descrizione)) {
      this.apiService.RestoreTitolario(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
