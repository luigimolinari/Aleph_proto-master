...
@Component({
  selector: 'app-view-tipologia-documentazione',
  templateUrl: './view-tipologia-documentazione.component.html',
  styleUrls: ['./view-tipologia-documentazione.component.css']
})
export class ViewTipologiaDocumentazioneComponent  {
  ...
  refresha() {
    ...
    this.apiService.getAllTipologiaDocumentazione().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  cancella(id, descrizione) {
    if (confirm("Stai per disattivare " + descrizione)) {
      this.apiService.DeleteTipologiaDocumentazione(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
  ...
  editMethod(e) {
    ...
    this.router.navigate(['/edittipologiadocumentazione'], navigationExtras1);
  }
  }
