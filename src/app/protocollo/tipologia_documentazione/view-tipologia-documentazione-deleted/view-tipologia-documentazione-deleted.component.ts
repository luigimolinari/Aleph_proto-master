...
@Component({
  selector: 'app-view-tipologia-documentazione-deleted',
  templateUrl: './view-tipologia-documentazione-deleted.component.html',
  styleUrls: ['./view-tipologia-documentazione-deleted.component.css']
})
export class ViewTipologiaDocumentazioneDeletedComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllTipologiaDocumentazioneDeleted().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  restore(id, descrizione) {
    if (confirm("Stai per riattivare " + descrizione)) {
      this.apiService.RestoreTipologiaDocumentazione(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
