...
@Component({
  ...
  styleUrls: ['./view-inbox.component.css']
})
export class ViewInboxComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllInBox().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  cancella(id, descrizione) {
    if (confirm("Stai per disattivare " + descrizione)) {
      this.apiService.DeleteInBox(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
  ...
  editMethod(e) {
    ...
    this.router.navigate(['/editinbox'], navigationExtras1);
  }
  }
