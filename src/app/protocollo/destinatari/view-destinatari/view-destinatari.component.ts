...
@Component({
  selector: 'app-view-destinatari',
  templateUrl: './view-destinatari.component.html',
  styleUrls: ['./view-destinatari.component.css']
})
export class ViewDestinatariComponent {
  ...
  refresha() {
    ...
    this.apiprotoService.getAllProtoDestinatari().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  cancella(id, nome) {
    if (confirm("Stai per disattivare " + nome)) {
      this.apiprotoService.DeleteProtoDestinatari(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
