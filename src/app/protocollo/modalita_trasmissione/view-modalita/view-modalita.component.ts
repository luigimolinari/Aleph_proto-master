...
@Component({
  selector: 'app-view-modalita',
  templateUrl: './view-modalita.component.html',
  styleUrls: ['./view-modalita.component.css']
})
export class ViewModalitaComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllModalita().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  cancella(id, descrizione) {
    if (confirm("Stai per disattivare " + descrizione)) {
      this.apiService.DeleteModalita(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
  ...
  editMethod(e) {
    ...
    this.router.navigate(['/editmodalita'], navigationExtras1);
  }
  }
