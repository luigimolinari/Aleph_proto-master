...
@Component({
  selector: 'app-view-modalita-deleted',
  templateUrl: './view-modalita-deleted.component.html',
  styleUrls: ['./view-modalita-deleted.component.css']
})
export class ViewModalitaDeletedComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllModalitaDeleted().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  restore(id, descrizione) {
    if (confirm("Stai per riattivare " + descrizione)) {
      this.apiService.RestoreModalita(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
