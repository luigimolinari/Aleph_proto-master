...
@Component({
  selector: 'app-view-inbox-deleted',
  templateUrl: './view-inbox-deleted.component.html',
  styleUrls: ['./view-inbox-deleted.component.css']
})
export class ViewInboxDeletedComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllInBoxDeleted().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  constructor(private http: HttpClient, private apiService: ApiService, private router: Router, private route: ActivatedRoute, private localStorageService: LocalStorageService) {
    ...
    this.customersTableColumns = [
      ...
      { name: 'mail', dataKey: 'is_mail', isSortable: true },
      { name: 'PEC', dataKey: 'is_pec', isSortable: true },
      { name: 'File', dataKey: 'is_file', isSortable: true },
  ...
  restore(id, descrizione) {
    if (confirm("Stai per riattivare " + descrizione)) {
      this.apiService.RestoreInBox(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
