...
@Component({
  selector: 'app-view-serie-deleted',
  templateUrl: './view-serie-deleted.component.html',
  styleUrls: ['./view-serie-deleted.component.css']
})
export class ViewSerieDeletedComponent {
  ...
  refresha() {
    ...
    this.apiService.getAllSerieDeleted().subscribe((data: any)=> {
      this.data = data;
    })
  }
  ...
  constructor(private http: HttpClient, private apiService: ApiService, private router: Router, private route: ActivatedRoute, private localStorageService: LocalStorageService,) {
    ...
    this.customersTableColumns = [
      ...
      { name: 'numerazione', dataKey: 'numerazione', isSortable: true },
      { name: 'annuale', dataKey: 'annuale', isSortable: true },
      { name: 'entrata_uscita', dataKey: 'entrata_uscita', isSortable: true },
      { name: 'interno', dataKey: 'interno', isSortable: true },
  ...
  restore(id, descrizione) {
    if (confirm("Stai per riattivare " + descrizione)) {
      this.apiService.RestoreSerie(id).subscribe((dati) => {
        if (dati['Esito'] === 'si') {
