...
@Component({
  selector: 'app-gruppo-validazione-pratica',
  templateUrl: './gruppo-validazione-pratica.component.html',
  styleUrls: ['./gruppo-validazione-pratica.component.css']
})
export class GruppoValidazionePraticaComponent implements OnInit {
    ...
    this.route.queryParams.subscribe(
      params => {
        ...
        this.apiService.getAllUsers('', '').subscribe((data: any) => {
          ...
          this.apiService.getGruppoValidazione(this.id_flusso, this.id_workflow).subscribe((data:any) => {
            ...
            if (this.originalOperatori != null) {
              ...
              this.form.controls.firma.setValue(data.req_ordine);
              this.original_firma = data.req_ordine;

              let original_mode = data.mod_firma;
  ...
  trasmetti() {
    ...
    if (this.originalOperatori == null) {
      ...
      if (this.selectedOperators.length == 0) {
        ...
        this.apiService.AddGruppoValidazionePratica(this.id_user, this.listaOperatori, this.id_flusso, this.id_workflow, this.mod, this.form.controls.firma.value, this.controlAtLeastOne, this.controlOrder).subscribe((dati) => {
          alert(dati['Esito']);
          this.back();
        });
      }
      ...
      if (selected_notchanged && done_notchanged && req_ordine_noctchanged && req_atleastone_notchanged && req_firma_notchanged) {
        ...
        if (confirm('Procedere con la modifica?')) {
          this.apiService.UpdateGruppoValidazionePratica(this.id_user, this.id_gruppo, this.listaOperatori, this.id_flusso, this.id_workflow, this.mod, this.form.controls.firma.value, this.controlAtLeastOne, this.controlOrder).subscribe((dati) => {
            alert(dati['Esito']);
            this.back();
          });
        }
