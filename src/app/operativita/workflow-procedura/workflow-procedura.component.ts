...
@Component({
  selector: 'app-workflow-procedura',
  templateUrl: './workflow-procedura.component.html',
  styleUrls: ['./workflow-procedura.component.css']
})
export class WorkflowProceduraComponent implements OnInit {
    ...
    if (step.isgateway == 1) {
      ...
      if (step.milestone == 1) {
        if (step.id_doc != '') {
          ...
          //form
          this.apiService.getFormValue(step.id_form, this.id_procedura).subscribe((result: any) => {
            if (result != null) {
  ...
  termina(): void{
   if(confirm("Procedre alla chiusura della procedura?")){
    this.apiService.CloseProcedura(this.id_procedura).subscribe((dati) => {
      alert(dati["Esito"]);
